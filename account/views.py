        ...
        if _new == _newagain and len(_new)>0:
            try:
                ...
                if user is not None and user.is_active:
                    ...
                    _user.save()
                    _success = "设置密码 "+ _username +" 完成"
                else:
                    _error = "原始密码错误!"
            except Exception as e:
                _error="设置密码 "+ _username +" 失败"
        else:
            _error="密码错误或两次密码不一致"
    return manage_user(request,success=_success,error=_error)
...
def setup_user(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            _success = "修改 user " + _username + " 完成"

        except Exception as e:
            _error ="修改 user " + _username + " 失败" 
...
def modify_business(request):
    ...
    if request.method=="POST":
        ...
        try:
            ...
            _business.save()
            _success="修改业务 "+ _name +" 完成"
        except Exception as e:
            _error="修改业务 "+ _name +" 失败"
...
def modify_privilege(request):
    ...
    if request.method=="POST":
        ...
        try:
            ...
            _privilege.save()
            _success="修改 "+ _name +" 权限完成"
        except Exception as e:
            _error="修改 "+ _name +" 权限失败"
