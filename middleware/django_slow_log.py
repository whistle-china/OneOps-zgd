...
#!/bin/env python
'''
record slow url customer accessed 
...
DJANGO_SLOW_LOGGER = custom_logger_constructor(
	"django_slow", 
)
			...
			_message = '%s - - [%s] "%s %s %s" %s %s "%s" "%s" "%s" %sms' %\
				(   pretty_string(_host),
					...
					pretty_string(round(_time_delta*1000)) 
                        ...
                        if settings.DJANGO_SLOW_LOG_OPEN and int(_time_delta*1000) >=settings.DJANGO_SLOW_LOG_TIME_DELTA:
			    DJANGO_SLOW_LOGGER.info(_message)
			    DJANGO_SLOW_LOGGER.debug(_message)
